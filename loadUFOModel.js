...
import { currentUfoModelInGLTF, defaultModelUrl, setCurrentUfoModellInGLTF } from "./letall";
...
export async function loadUfoModel(modelUrl) {
    ...
    try {
        ...
        if (currentUfoModelInGLTF !== null) {
            scene.remove(currentUfoModelInGLTF);
            setCurrentUfoModellInGLTF (null); // Clear the reference to the removed model
        }
        ...
        setCurrentUfoModellInGLTF(gltf.scene); // Get the loaded model

        // Step 1: Calculate the model's bounding box
        const box = new THREE.Box3().setFromObject(currentUfoModelInGLTF);
        ...
        currentUfoModelInGLTF.scale.set(scaleFactor, scaleFactor, scaleFactor); // Scale the model uniformly

        // Check if the user has loaded their profile object
        if (modelUrl !== defaultModelUrl) {
            // Load user-specific settings if available
            const profileSettings = await loadProfileSettings(connectedAccount); // Load profile settings
            if (profileSettings.length === 0 || !profileSettings[0].position) {
                // If no settings loaded, use bounding box for position
                currentUfoModelInGLTF.position.set(box.min.x, box.min.y, box.min.z); // Use bounding box min for position
                currentUfoModelInGLTF.scale.set(scaleFactor, scaleFactor, scaleFactor); // Use calculated scale factor
                // Step 5: Position the model within the bounding box
                currentUfoModelInGLTF.position.set(0, -5, -30); // Adjust as needed
            } else {
                // Apply user-specific settings
                currentUfoModelInGLTF.position.set(
                ...
                currentUfoModelInGLTF.rotation.set(
                ...
                currentUfoModelInGLTF.scale.set(userScaleFactor, userScaleFactor, userScaleFactor); // Scale the model uniformly
            }
        } else {
            // Default settings for new users
            currentUfoModelInGLTF.position.set(0, -3, -8); // Default position
            currentUfoModelInGLTF.rotation.set(
            ...
            currentUfoModelInGLTF.scale.set(defaultScaleFactor, defaultScaleFactor, defaultScaleFactor); // Scale the model uniformly
        }
        ...
        document.getElementById("rotation-x").addEventListener("input", (event) => {
            ...
            currentUfoModelInGLTF.rotation.x = THREE.MathUtils.degToRad(value); // Convert degrees to radians
        });
        ...
        document.getElementById("rotation-y").addEventListener("input", (event) => {
            ...
            currentUfoModelInGLTF.rotation.y = THREE.MathUtils.degToRad(value); // Convert degrees to radians
        });
        ...
        document.getElementById("rotation-z").addEventListener("input", (event) => {
            ...
            currentUfoModelInGLTF.rotation.z = THREE.MathUtils.degToRad(value); // Convert degrees to radians
        });
        ...
        document.getElementById("position-x").addEventListener("input", (event) => {
            ...
            currentUfoModelInGLTF.position.x = parseFloat(value); // Update the x position of the model
        });
        ...
        document.getElementById("position-y").addEventListener("input", (event) => {
            ...
            currentUfoModelInGLTF.position.y = parseFloat(value); // Update the y position of the model
        });
        ...
        document.getElementById("position-z").addEventListener("input", (event) => {
            ...
            currentUfoModelInGLTF.position.z = parseFloat(value); // Update the z position of the model
        });
        ...
        document.getElementById("position-x").value = currentUfoModelInGLTF.position.x;
        document.getElementById("position-y").value = currentUfoModelInGLTF.position.y;
        document.getElementById("position-z").value = currentUfoModelInGLTF.position.z;
        document.getElementById("scale-factor").value = currentUfoModelInGLTF.scale.x;
        document.getElementById("position-x-value").value = currentUfoModelInGLTF.position.x;
        document.getElementById("position-y-value").value = currentUfoModelInGLTF.position.y;
        document.getElementById("position-z-value").value = currentUfoModelInGLTF.position.z;

        const rotationXInDegrees = THREE.MathUtils.radToDeg(currentUfoModelInGLTF.rotation.x);
        const rotationYInDegrees = THREE.MathUtils.radToDeg(currentUfoModelInGLTF.rotation.y);
        const rotationZInDegrees = THREE.MathUtils.radToDeg(currentUfoModelInGLTF.rotation.z);
        ...
        document.getElementById("scale-factor-value").value = currentUfoModelInGLTF.scale.x;
